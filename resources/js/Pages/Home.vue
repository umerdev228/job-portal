...
const filteredjobs = computed(() => {
    ...
    if (!selectedCategory) return jobs.filter(job => job.is_feature);
    // Filter jobs by selected category and feature status
    return jobs.filter(job => job.category_id === selectedCategory.id && job.is_feature);
});
...
// Method to show feature jobs by category
const showFeatureJobsByCategory = (category) => {
    ...
    <FrontendLayout :auth="auth">
        ...
        <div class="flex items-center justify-end py-4 md:py-8 flex-wrap">
            ...
            <button v-for="category in categories"
                   @click="showFeatureJobsByCategory(category)"
