    ...
    <AdminLayout :auth="auth" :notifications="notifications">
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                    <tr v-for="role in roles.data" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <td class="px-6 py-4 text-right flex">
                            <Link class="font-medium text-blue-600 dark:text-blue-500 hover:underline" :href="route('admin.roles.edit', role.id)">
                                <svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
                                    <path fill-rule="evenodd" d="M14 4.182A4.136 4.136 0 0 1 16.9 3c1.087 0 2.13.425 2.899 1.182A4.01 4.01 0 0 1 21 7.037c0 1.068-.43 2.092-1.194 2.849L18.5 11.214l-5.8-5.71 1.287-1.31.012-.012Zm-2.717 2.763L6.186 12.13l2.175 2.141 5.063-5.218-2.141-2.108Zm-6.25 6.886-1.98 5.849a.992.992 0 0 0 .245 1.026 1.03 1.03 0 0 0 1.043.242L10.282 19l-5.25-5.168Zm6.954 4.01 5.096-5.186-2.218-2.183-5.063 5.218 2.185 2.15Z" clip-rule="evenodd"/>
                                </svg>
                            </Link>
                            <Link class="font-medium text-blue-600 dark:text-blue-500 hover:underline" method="delete" :href="route('admin.roles.destroy', role.id)">
        ...
        <nav aria-label="Page navigation example" class="mt-6">
            <ul class="inline-flex -space-x-px text-sm">
                <li>
                    <Component :is="roles.links[0].url ? Link : 'span'"
                               :class="{ 'font-bold': roles.links[0].active, 'hover:text-white hover:font-bold': roles.links[0].active }"
                               ...
                               v-html="roles.links[0].label"/>
                </li>
                <li v-for="(link, index) in roles.links.slice(1, -1)" :key="index">
                ...
                <li>
                    <Component :is="roles.links.slice(-1)[0].url ? Link : 'span'"
                               :class="{ 'font-bold': roles.links.slice(-1)[0].active, 'hover:text-white hover:font-bold': roles.links.slice(-1)[0].active }"
                               :href="roles.links.slice(-1)[0].url"
                               class="flex items-center justify-center px-3 h-8 leading-tight text-gray-500 bg-white border border-gray-300 rounded-r-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white"
                               v-html="roles.links.slice(-1)[0].label"/>
                </li>
            </ul>
        </nav>
